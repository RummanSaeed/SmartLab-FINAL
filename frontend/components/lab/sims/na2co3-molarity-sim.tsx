...
// Burette with standard acid solution
  ...
  return (
    <group position={[0, 2, 0]}>
      ...
      <mesh position={[0, -1.6, 0]}>
        <cylinderGeometry args={[0.08, 0.08, 0.2, 16]} rotation={[0, 0, Math.PI / 2]} />
...
// Conical flask with Na2CO3 solution and indicator
function ConicalFlask({ 
  indicatorColor, isTitrating 
}: { 
...
export function Na2CO3MolaritySim() {
  const [addedVolume, setAddedVolume] = useState(0)
  const [isTitrating, setIsTitrating] = useState(false)
  const [indicatorColor, setIndicatorColor] = useState("#ff8c00") // Methyl orange in base
  ...
  const getIndicatorColor = (vol: number) => {
    if (vol < 18) return "#ffd700" // Yellow (basic)
    if (vol < 20) return "#ffcc00" // Yellow-orange (near endpoint)
    if (vol < 22) return "#ffa500" // Orange (endpoint)
    return "#ff4500" // Orange-red (acidic, past endpoint)
  }
    ...
    const interval = setInterval(() => {
      setAddedVolume((prev) => {
        ...
        if (newVol >= 25) {
          ...
          // M(Na2CO3) = M(HCl) * V(HCl) / (2 * V(Na2CO3))
    ...
    <div className="h-full flex flex-col gap-4 overflow-auto pr-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Acid Added</span>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium">
                {indicatorColor === "#ffd700" ? "Yellow" : 
                 indicatorColor === "#ffcc00" ? "Yellow-Orange" :
                 indicatorColor === "#ffa500" ? "Orange" : "Orange-Red"}
              </span>
            </div>
      ...
      <div className="rounded-xl border border-border/60 bg-card/40 p-4 text-sm space-y-2">
        ...
        <p className="text-muted-foreground">
          The molarity of Na₂CO₃ solution is determined by titration against standard HCl using methyl orange indicator.
          At the endpoint, the color changes from yellow to orange.
        </p>
        <p className="text-muted-foreground mt-2">
          <span className="font-medium">Reaction:</span> Na₂CO₃ + 2HCl → 2NaCl + H₂O + CO₂↑
        </p>
        <p className="text-muted-foreground">
          <span className="font-medium">Formula:</span> M(Na₂CO₃) = M(HCl) × V(HCl) / (2 × V(Na₂CO₃))
        </p>
      </div>
    </div>
  )
