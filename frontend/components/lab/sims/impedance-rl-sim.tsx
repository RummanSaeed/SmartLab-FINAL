...
type Trial = {
  ...
  measuredInductance: number
}

// Inductor with magnetic field visualization
function Inductor({ 
  current, 
  frequency, 
  inductance 
  ...
  const inductorRef = useRef<THREE.Group>(null)
  const fieldRef = useRef<THREE.Mesh>(null)
  
  useFrame(() => {
    if (inductorRef.current) {
      // Animate magnetic field based on current
      const time = Date.now() * 0.001
      const fieldStrength = Math.sin(2 * Math.PI * frequency * time) * current
      
      inductorRef.current.children.forEach((coil, index) => {
        if (coil instanceof THREE.Mesh) {
          const material = coil.material as THREE.MeshStandardMaterial
          material.emissiveIntensity = Math.abs(fieldStrength) * 0.5
        }
      })
    }
    ...
    if (fieldRef.current) {
      ...
      material.emissiveIntensity = Math.abs(fieldStrength) * 0.2
    }
  })
  ...
  return (
    <group position={[0, 0.5, 0]}>
      ...
      <mesh position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color="#696969" metalness={0.9} />
      </mesh>
  ...
  return (
    <group position={[0, 3.5, 0]}>
      ...
      <Text position={[0, -0.65, 0.07]} fontSize={0.04} color="#00ff00" anchorX="center">
        R = {resistance}Ω, L = {(inductance * 1000).toFixed(1)}mH
      </Text>
...
export function ImpedanceRLSim({ 
  resistance: initialResistance = 100, 
  inductance: initialInductance = 0.1, 
  frequency: initialFrequency = 50
}: Props) {
  const [resistance] = useState(initialResistance)
  const [inductance] = useState(initialInductance)
  ...
  const calculateImpedance = (freq: number): number => {
    const inductiveReactance = 2 * Math.PI * freq * inductance
    return Math.sqrt(resistance * resistance + inductiveReactance * inductiveReactance)
  }
  ...
  const completeMeasurement = () => {
    if (dataPoints.length > 0) {
      // Calculate inductance from impedance data
      ...
      const measuredInductance = Math.sqrt(avgImpedance * avgImpedance - resistance * resistance) / (2 * Math.PI * avgFreq)
  ...
  const autoSweep = () => {
    ...
    frequencies.forEach(freq => {
      ...
      const indReact = 2 * Math.PI * freq * inductance
      const phase = Math.atan(indReact / resistance) * (180 / Math.PI)
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-auto pr-1">
      <div className="flex flex-wrap gap-2">
        ...
        <Badge variant="outline">Inductance: {(inductance * 1000).toFixed(1)} mH</Badge>
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
        Measure inductance by studying impedance variation with frequency.
        In RL circuits, impedance Z = √(R² + (2πfL)²) increases with frequency.
        Plot impedance vs frequency to determine inductance.
      </div>
      ...
      {dataPoints.length > 0 && (
        <div className="rounded-lg border border-border/60 bg-card/40 p-3">
          ...
          <div className="grid grid-cols-5 gap-2 text-xs">
            ...
            <div className="text-muted-foreground">X_L (Ω)</div>
            <div className="text-muted-foreground">L_calc (mH)</div>
            {dataPoints.slice(-5).map((point, i) => (
              <div key={i} className="contents">
                ...
                <div>{(2 * Math.PI * point.frequency * inductance).toFixed(1)}</div>
                <div>{(Math.sqrt(point.impedance * point.impedance - resistance * resistance) / (2 * Math.PI * point.frequency) * 1000).toFixed(2)}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {trials.map((trial, i) => (
              <div key={i} className="text-xs">
                ...
                <div>Points: {trial.dataPoints.length} | Actual: {(trial.inductance * 1000).toFixed(1)} mH | Measured: {(trial.measuredInductance * 1000).toFixed(1)} mH</div>
              </div>
            ))}
          </div>
        </div>
