"use client"

import { useState, useRef, useMemo } from "react"
import { Canvas, useFrame } from "@react-three/fiber"
import { OrbitControls, Text, Html } from "@react-three/drei"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Play, RotateCcw, Beaker, Settings2 } from "lucide-react"
import * as THREE from "three"

// Burette with NaOH solution
type BuretteProps = {
  fillLevel: number
  isDripping: boolean
}

function Burette({ fillLevel, isDripping }: BuretteProps) {
  const dripRef = useRef<THREE.Mesh>(null)
  const dropRef = useRef<THREE.Mesh>(null)
  const [dropY, setDropY] = useState(0)

  useFrame((state) => {
    if (isDripping && dropRef.current) {
      const time = state.clock.elapsedTime * 2
      dropRef.current.position.y = -0.8 - (time % 1) * 0.5
      dropRef.current.visible = (time % 1) < 0.8
    }
  })

  return (
    <group position={[0, 2, 0]}>
      {/* Burette body - graduated cylinder */}
      <mesh>
        <cylinderGeometry args={[0.15, 0.15, 3, 32]} />
        <meshPhysicalMaterial
          color="rgba(200, 220, 240, 0.3)"
          transmission={0.8}
          roughness={0.1}
          thickness={0.1}
          transparent
          opacity={0.3}
        />
      </mesh>
      
      {/* NaOH solution inside */}
      <mesh position={[0, -1.5 + fillLevel * 1.5, 0]}>
        <cylinderGeometry args={[0.13, 0.13, fillLevel * 3, 32]} />
        <meshStandardMaterial
          color="#f0f0f0"
          transparent
          opacity={0.9}
        />
      </mesh>

      {/* Graduation marks */}
      {Array.from({ length: 30 }).map((_, i) => (
        <mesh key={i} position={[0.16, 1.4 - i * 0.09, 0]}>
          <boxGeometry args={[0.02, 0.005, 0.005]} />
          <meshStandardMaterial color="#333" />
        </mesh>
      ))}

      {/* Stopcock */}
      <mesh position={[0, -1.6, 0]} rotation={[0, 0, Math.PI / 2]}>
        <cylinderGeometry args={[0.08, 0.08, 0.2, 16]} />
        <meshStandardMaterial color="#444" />
      </mesh>

      {/* Tip */}
      <mesh position={[0, -1.8, 0]}>
        <coneGeometry args={[0.04, 0.15, 16]} />
        <meshStandardMaterial color="#666" />
      </mesh>

      {/* Falling drop */}
      <mesh ref={dropRef} position={[0, -0.8, 0]} visible={false}>
        <sphereGeometry args={[0.03, 16, 16]} />
        <meshStandardMaterial color="#f0f0f0" transparent opacity={0.9} />
      </mesh>
    </group>
  )
}

// Conical flask with acid and indicator
function ConicalFlask({ 
  volume, 
  indicatorColor,
  isTitrating 
}: { 
  volume: number
  indicatorColor: string
  isTitrating: boolean 
}) {
  const liquidRef = useRef<THREE.Mesh>(null)
  const swirlRef = useRef<THREE.Group>(null)

  useFrame((state) => {
    if (isTitrating && swirlRef.current) {
      const t = state.clock.elapsedTime * 3
      swirlRef.current.rotation.y = Math.sin(t) * 0.1
      swirlRef.current.position.x = Math.sin(t * 2) * 0.02
    }
  })

  return (
    <group position={[0, -0.5, 0]}>
      {/* Flask body */}
      <mesh>
        <coneGeometry args={[0.6, 0.8, 32]} />
        <meshPhysicalMaterial
          color="rgba(220, 235, 255, 0.2)"
          transmission={0.9}
          roughness={0.05}
          thickness={0.15}
          transparent
          opacity={0.25}
        />
      </mesh>

      {/* Flask neck */}
      <mesh position={[0, 0.6, 0]}>
        <cylinderGeometry args={[0.15, 0.15, 0.4, 32]} />
        <meshPhysicalMaterial
          color="rgba(220, 235, 255, 0.2)"
          transmission={0.9}
          roughness={0.05}
          thickness={0.15}
          transparent
          opacity={0.25}
        />
      </mesh>

      {/* Solution with indicator */}
      <group ref={swirlRef}>
        <mesh ref={liquidRef} position={[0, -0.2, 0]}>
          <coneGeometry args={[0.55, 0.7, 32]} />
          <meshStandardMaterial
            color={indicatorColor}
            transparent
            opacity={0.85}
            roughness={0.2}
          />
        </mesh>

        {/* Phenolphthalein indicator molecules effect */}
        {isTitrating && Array.from({ length: 8 }).map((_, i) => (
          <mesh
            key={i}
            position={[
              Math.sin(i * 0.8) * 0.3,
              -0.3 + Math.cos(i * 0.5) * 0.2,
              Math.cos(i * 0.8) * 0.3
            ]}
          >
            <sphereGeometry args={[0.015, 8, 8]} />
            <meshStandardMaterial
              color={indicatorColor === "#ff69b4" ? "#ff1493" : "#ffb6c1"}
              emissive={indicatorColor === "#ff69b4" ? "#ff1493" : "#ffb6c1"}
              emissiveIntensity={0.5}
            />
          </mesh>
        ))}
      </group>

      {/* Volume marking (25 mL) */}
      <mesh position={[0.45, -0.1, 0]} rotation={[0, Math.PI / 2, 0]}>
        <torusGeometry args={[0.02, 0.002, 8, 16]} />
        <meshStandardMaterial color="#444" />
      </mesh>

      {/* White tile underneath */}
      <mesh position={[0, -1, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[2, 2]} />
        <meshStandardMaterial color="#f8f8f8" />
      </mesh>
    </group>
  )
}

// Bunsen burner with blue flame
function BunsenBurner({ isHeating }: { isHeating: boolean }) {
  const flameRef = useRef<THREE.Group>(null)

  useFrame((state) => {
    if (isHeating && flameRef.current) {
      const t = state.clock.elapsedTime * 15
      flameRef.current.children.forEach((child, i) => {
        if (child instanceof THREE.Mesh) {
          child.scale.y = 1 + Math.sin(t + i) * 0.15
          child.position.x = Math.sin(t * 0.5 + i) * 0.01
        }
      })
    }
  })

  return (
    <group position={[0, -1.2, 0]}>
      {/* Burner base */}
      <mesh position={[0, -0.3, 0]}>
        <cylinderGeometry args={[0.2, 0.25, 0.4, 24]} />
        <meshStandardMaterial color="#2d3748" metalness={0.8} roughness={0.3} />
      </mesh>

      {/* Burner tube */}
      <mesh position={[0, 0.1, 0]}>
        <cylinderGeometry args={[0.08, 0.08, 0.5, 24]} />
        <meshStandardMaterial color="#4a5568" metalness={0.7} roughness={0.4} />
      </mesh>

      {/* Blue flame */}
      <group ref={flameRef}>
        <mesh position={[0, 0.45, 0]} visible={isHeating}>
          <coneGeometry args={[0.06, 0.25, 16]} />
          <meshStandardMaterial
            color="#4169e1"
            emissive="#1e90ff"
            emissiveIntensity={2}
            transparent
            opacity={0.7}
          />
        </mesh>
        <mesh position={[0, 0.35, 0]} visible={isHeating}>
          <coneGeometry args={[0.04, 0.2, 16]} />
          <meshStandardMaterial
            color="#00bfff"
            emissive="#87ceeb"
            emissiveIntensity={3}
            transparent
            opacity={0.8}
          />
        </mesh>
      </group>

      {/* Gas tube connection */}
      <mesh position={[-0.3, -0.5, 0]} rotation={[0, 0, -Math.PI / 6]}>
        <cylinderGeometry args={[0.03, 0.03, 0.4, 12]} />
        <meshStandardMaterial color="#1a202c" />
      </mesh>
    </group>
  )
}

// Lab table surface
function LabTable() {
  return (
    <mesh position={[0, -1.5, 0]} rotation={[-Math.PI / 2, 0, 0]}>
      <planeGeometry args={[8, 6]} />
      <meshStandardMaterial color="#334155" roughness={0.8} />
    </mesh>
  )
}

export function NaOHStandardizationSim() {
  const [buretteVolume, setBuretteVolume] = useState(50)
  const [flaskVolume, setFlaskVolume] = useState(25)
  const [addedVolume, setAddedVolume] = useState(0)
  const [isTitrating, setIsTitrating] = useState(false)
  const [indicatorColor, setIndicatorColor] = useState("#ffffff")
  const [endpointReached, setEndpointReached] = useState(false)
  const [molarity, setMolarity] = useState<number | null>(null)

  const buretteFill = (50 - addedVolume) / 50
  
  // Calculate indicator color based on pH (simplified)
  const getIndicatorColor = (vol: number) => {
    // Phenolphthalein: colorless in acid, pink in base
    // Endpoint around 15-20 mL of NaOH added
    if (vol < 15) return "#ffffff" // Colorless (acidic)
    if (vol < 17) return "#ffe4e1" // Very light pink (approaching endpoint)
    if (vol < 19) return "#ffb6c1" // Light pink (near endpoint)
    return "#ff69b4" // Deep pink (basic, endpoint reached)
  }

  const startTitration = () => {
    setIsTitrating(true)
    setEndpointReached(false)
    
    const interval = setInterval(() => {
      setAddedVolume((prev) => {
        const newVol = prev + 0.2
        const newColor = getIndicatorColor(newVol)
        setIndicatorColor(newColor)
        
        if (newVol >= 17 && newVol < 19 && !endpointReached) {
          setEndpointReached(true)
        }
        
        if (newVol >= 25) {
          clearInterval(interval)
          setIsTitrating(false)
          // Calculate molarity: M1V1 = M2V2
          // 0.1 M HCl * 25 mL = M2 * 17.5 mL (avg endpoint)
          const calculatedMolarity = (0.1 * 25) / 17.5
          setMolarity(calculatedMolarity)
        }
        
        return newVol
      })
    }, 100)
  }

  const resetSimulation = () => {
    setAddedVolume(0)
    setIsTitrating(false)
    setIndicatorColor("#ffffff")
    setEndpointReached(false)
    setMolarity(null)
    setBuretteVolume(50)
  }

  return (
    <div className="h-full flex flex-col gap-4 overflow-auto pr-1">
      <div className="flex flex-wrap gap-2">
        <Badge variant="outline" className="flex items-center gap-1">
          <Beaker className="w-3 h-3" />
          NaOH Standardization
        </Badge>
        <Badge variant={endpointReached ? "default" : "outline"}>
          {endpointReached ? "Endpoint Detected!" : "Titration in Progress"}
        </Badge>
        {molarity && (
          <Badge variant="default" className="bg-green-600">
            Calculated Molarity: {molarity.toFixed(4)} M
          </Badge>
        )}
      </div>

      <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden h-[400px]">
        <Canvas camera={{ position: [3, 2, 4], fov: 50 }}>
          <color attach="background" args={["#020817"]} />
          <ambientLight intensity={0.6} />
          <directionalLight position={[5, 10, 5]} intensity={1.2} castShadow />
          <pointLight position={[-5, 5, -5]} intensity={0.5} />
          
          <LabTable />
          <Burette fillLevel={buretteFill} isDripping={isTitrating} />
          <ConicalFlask 
            volume={flaskVolume} 
            indicatorColor={indicatorColor}
            isTitrating={isTitrating}
          />
          <BunsenBurner isHeating={false} />

          {/* Labels */}
          <Text
            position={[0.5, 3.5, 0]}
            fontSize={0.15}
            color="#1a202c"
            anchorX="center"
          >
            NaOH Burette
          </Text>
          <Text
            position={[0.8, -0.3, 0]}
            fontSize={0.12}
            color="#1a202c"
            anchorX="left"
          >
            HCl + Phenolphthalein
          </Text>
          <Text
            position={[-0.5, 1.8, 0]}
            fontSize={0.1}
            color="#4a5568"
            anchorX="center"
          >
            {addedVolume.toFixed(1)} mL added
          </Text>

          <OrbitControls 
            enablePan={false} 
            minDistance={3} 
            maxDistance={8}
            maxPolarAngle={Math.PI / 2}
          />
        </Canvas>
      </div>

      {/* Controls */}
      <div className="space-y-4 p-4 rounded-lg border border-primary/20 bg-primary/5">
        <div className="flex items-center gap-2 text-sm font-medium text-primary">
          <Settings2 className="w-4 h-4" />
          Titration Controls
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Volume Added</span>
              <span className="font-medium">{addedVolume.toFixed(1)} mL</span>
            </div>
            <div className="h-2 bg-muted rounded-full overflow-hidden">
              <div 
                className="h-full bg-primary transition-all duration-300"
                style={{ width: `${(addedVolume / 25) * 100}%` }}
              />
            </div>
          </div>

          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Solution Color</span>
              <span className="font-medium">{indicatorColor === "#ffffff" ? "Colorless" : endpointReached ? "Deep Pink" : "Light Pink"}</span>
            </div>
            <div 
              className="h-6 rounded border border-gray-300 transition-colors duration-500"
              style={{ backgroundColor: indicatorColor }}
            />
          </div>
        </div>

        <div className="flex gap-2">
          <Button
            onClick={startTitration}
            disabled={isTitrating || addedVolume >= 25}
            className="flex-1"
          >
            <Play className="w-4 h-4 mr-2" />
            {isTitrating ? "Titrating..." : "Start Titration"}
          </Button>
          <Button
            variant="outline"
            onClick={resetSimulation}
            disabled={isTitrating}
          >
            <RotateCcw className="w-4 h-4 mr-2" />
            Reset
          </Button>
        </div>
      </div>

      <div className="rounded-xl border border-border/60 bg-card/40 p-4 text-sm space-y-2">
        <p className="font-medium text-foreground">Principle:</p>
        <p className="text-muted-foreground">
          NaOH (base) is standardized against a known concentration of HCl (acid) using 
          phenolphthalein indicator. At the endpoint, the solution changes from colorless to pink.
        </p>
        <p className="text-muted-foreground mt-2">
          <span className="font-medium">Reaction:</span> NaOH + HCl → NaCl + H₂O
        </p>
        <p className="text-muted-foreground">
          <span className="font-medium">Formula:</span> M₁V₁ = M₂V₂
        </p>
      </div>
    </div>
  )
}
